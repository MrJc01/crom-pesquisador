...
deploy_fleet() {
    ...
    for file in "$TARGETS_DIR"/*.json; do
        if [ -f "$file" ]; then
            ...
            if [ "$is_running" -eq 0 ]; then
                ...
                nohup bash -c "cd '$PROJECT_ROOT' && go run crawler/main.go --config 'crawler/targets/$filename' >> '$LOGS_DIR/fleet_$filename.log' 2>&1" &>/dev/null &
                sleep 0.5
            else
    ...
    # Mover arquivos que foram salvos no diretório errado pela versão anterior
    if [ -d "$PROJECT_ROOT/crawler/data" ]; then
        echo -e "${C_YELLOW}Sincronizando bancos de dados isolados para a raiz do projeto...${C_RESET}"
        mkdir -p "$PROJECT_ROOT/data"
        cp -r "$PROJECT_ROOT/crawler/data/"* "$PROJECT_ROOT/data/" 2>/dev/null || true
