...
func IndexNode(detail *LinkDetail) {
	...
	_, err = globalDB.Exec(`
		INSERT INTO search_index (id, domain, url, title, description, keywords)
		VALUES (?, ?, ?, ?, ?, ?)
		ON CONFLICT(url) DO UPDATE SET
			title=excluded.title, description=excluded.description, keywords=excluded.keywords;
	`, detail.ID, detail.Technical.Domain, detail.URL, detail.Meta.Title, detail.Meta.Description, string(keywordsBytes))
